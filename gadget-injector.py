...
def main(argv):
    try:
        opts, args = getopt.getopt(argv, "ha:c:g:r:", [
            "help", "target-apk=", "proxy-cert=", "frida-gadget=",
            "device-arch"
        ])
        ...
        tamperApp(OUTPUT_DIR, FRIDA_GADGET, ACTIVITY_NAME, APK, DEVICE_ARCH,
                  PEM)
...
def convertDerToPem(cert):
    try:
        ...
        convertToPemThread = threading.Thread(
            target=worker,
            kwargs=dict(cmd="openssl",
                        args=[
                            "x509", "-inform", "der", "-in", cert, "-out",
                            proxyCertPem
                        ],
                        message="[i] Converting DER to PEM..."))
...
def decodeApp(apk):
    try:
        apktoolThread = threading.Thread(
            target=worker,
            kwargs=dict(cmd="apktool",
                        args=["d", apk, "-f"],
                        message="[i] Decoding with apktool..."))
        apktoolThread.start()
        apktoolThread.join()
                    ...
                    for activity in activitiesTag:
                        ...
                        if len(intentFilterTag) > 0:
                            for intent in intentFilterTag:
                                actionTag = intent.getElementsByTagName(
                                    "action")
                                if len(actionTag) > 0:
                                    for action in actionTag:
                                        if action.hasAttribute("android:name"):
                                            if "MAIN" in action.attributes[
                                                    "android:name"].value:
                                                activityName = activity.attributes[
                                                    "android:name"].value
                                                break
...
def tamperApp(outputDir, fridaGadget, activityName, apk, deviceArch, pem):
    try:
        if os.path.exists(fridaGadget):
                ...
                if os.path.isfile(smaliFilePath):
                    ...
                    for line in smaliFileLines:
                        if ".method static constructor" in line or ".method public constructor" in line and not patched:
                            ...
                            nextLine = smaliFileLines[
                                smaliFileLines.index(line) + 1]
                            newLocalsNumber = 1
                            ...
                            if ".locals" in nextLine:
                                ...
                                nextLine = nextLine.replace(
                                    str(localsNumber), str(newLocalsNumber))
                            ...
                            smaliFileUpdated.write('\n')
                            fridaGadgetLine = "    const-string v" + str(
                                newLocalsNumber - 1) + ", \"frida-gadget\""
                            smaliFileUpdated.write(fridaGadgetLine)
                            smaliFileUpdated.write('\n')
                            fridaLibCallLine = "    invoke-static {v" + str(
                                newLocalsNumber - 1
            ...
            #BUILD TAMPERED APP
            apktoolBuildThread = threading.Thread(
                target=worker,
                kwargs=dict(cmd="apktool",
                            args=["b", outputDir],
                            message="[i] Re-building application"))
            ...
            if os.path.isfile(rebuildedAppPath):
                zipalignThread = threading.Thread(
                    target=worker,
                    kwargs=dict(cmd="zipalign",
                                args=[
                                    "-p", "4", rebuildedAppPath,
                                    distDir + outputDir + "-aligned.apk"
                                ],
                                message="[i] Zipaligning re-builded app"))
            ...
            createKeystoreThread = threading.Thread(
                target=worker,
                kwargs=dict(
                    cmd="keytool",
                    args=[
                        "-genkey", "-alias", "keystore", "-dname",
                        "CN=sslunpinner, OU=SU, O=SU, L=su, S=su, C=su",
                        "-keystore", keyStore, "-storetype", "PKCS12",
                        "-keyalg", "RSA", "-storepass", prngValue, "-validity",
                        "1", "-keysize", "2048"
                    ],
                    message="[+] Creating keystore..."))
            ...
            if os.path.isfile(alignedAppPath + apkExt):
                signThread = threading.Thread(
                    target=worker,
                    kwargs=dict(cmd="apksigner",
                                args=[
                                    "sign", "--ks", keyStore, "--ks-pass",
                                    "pass:" + prngValue, "--key-pass",
                                    "pass:" + prngValue,
                                    alignedAppPath + apkExt
                                ],
                                message="[+] Signing with apksigner..."))
                signThread.start()
                signThread.join()
                ...
                if skipLine:
                    ...
                    if "BEGIN CERTIFICATE" in line:
                        ...
                        for certLine in certLines:
                            if certLine == certLines[
                                    0] or certLine == certLines[
                                        len(certLines) - 2] or certLine == "":
                                continue
                            else:
                                fridaScriptFileUpdated.write("\t\t+ \"" +
                                                             certLine +
                                                             "\\n\"" + "\n")
                        skipLine = True
...
def usage():
 ...
 |_||_| |_|_/ | \___| \___| \__|\___/ |_|   
    ...
    print "\nUSAGE:\t" + sys.argv[0] + " -h"
    print "\t" + sys.argv[
        0] + " -a app.apk" + " -c cacert.der" + " -g frida-gadget-12.2.26-android-x86.so" + " -r x86\n"
